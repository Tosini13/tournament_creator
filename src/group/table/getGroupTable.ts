...
const convertMapToArray = (map: TMapGroup): TGroupTable =>
...
export const getGroupTable = (group: TGroup, options?: TSortOptions): TGroupTable => {
  ...
  const countedTable: TMapGroup = matchesWithScoreAndTeams.reduce<TMapGroup>(
    (map, match) => countTable(match)(map),
    new Map<Id, TMapArg>(group.teams.map((team) => [team, initRow])),
  );
