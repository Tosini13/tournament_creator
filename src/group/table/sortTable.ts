...
const sortByScoredGoals =
  (next?: TSortTable): TSortTable =>
  (rowA, rowB) => {
...
const sortByDirectScore =
  (matches: TMatchWithScoreAndTeams[]) =>
  (options?: TSortOptions): TSortTable =>
  (rowA, rowB) => {
    const teamsMatches = matches.filter(
      (match) =>
        (match.homeTeam === rowA.team || match.homeTeam === rowB.team) &&
        (match.awayTeam === rowA.team || match.awayTeam === rowB.team),
    ); // TODO: check ID

    const goals = teamsMatches.reduce(
      (acc, match) => ({
        [match.homeTeam]: {
...
export const sortTable =
  ...
  (options?: TSortOptions): TSortTable =>
  (rowA, rowB) =>
    sortByPoints(sortByWonMatches(sortByGoalsDiffrence(sortByScoredGoals(sortByDirectScore(matches)(options)))))(
      rowA,
      rowB,
    );
